...
*/
	...
	public List<Adviceview> getAdviceId() {
		...
		String sql = "select id from soonface2db.Advice";
		...
		for(int i=0; i<list.size();i++) {
			...
			//insert
			update2(list.get(i),getAdviceImgsrc(list.get(i)),getAdviceTitle2(list.get(i)));
	...
	public int update2(String id, String content, String title) {
		...
		String sql = "update Adviceview set id=?,content=?,title=? where id =? ";
	...
	@Override
	public String getAdviceImgsrc(String id) {
		
		//그럼 여기서 그 id랑 content랑 같이 list에 저장.
		String sql = "select content from soonface2db.Advice where id= ?";
	...
	@Override
	public String getAdviceTitle2(String id) {
		String sql = "select title from soonface2db.Advice where id= ?";
